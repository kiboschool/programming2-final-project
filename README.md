...
In particular right now it will break, a lot. We will improve on it over the course of the milestones
...
Your first task is to write unit tests for _every_ function defined in untested_helpers.py.
...
2. We then ask the for the participant count - this may be useful, as for our style of tournament to work, we need the number of participants to be a power of two
3. We then create an empty list?
4. Then we ask our tournamen object to create the games using that empty list.

Clearly that will not work. Let's fix these two issues:
...
This method should look for the right file. If we want to load the _summer_event_ tournament, then we will look for `summer_event.games`

Once found, we should read and recreate all the games stored in the file. You can use the `from_json_string` method of the `Game` class to support with that.
...
Once you have `apikey.txt` set up, go ahead and run `event_brite_client.py`. If everything went well, you should see:
- Some string showing up in the console - that is the id of the event we just created
- If you visit your event brite page [here](https://www.eventbrite.com/organizations/home), you should see that a Test event was scheduled for May 2024!

## Part 2: Updating events

We provided a function to you in order to create events - You are welcome!

It is also here to serve as a _reference_, so you can create the function to **update** events on your own!
...
- Then build up the `update_event` method. Note that the URL you need to send the request to is documented [here](https://www.eventbrite.com/platform/api#/reference/event/update/update-an-event)
...
In order to set up events for the tournament, we have created a file called `scheduler.py`. Functions within this file will receive games as input, and call the right methods from `EventBriteAPIHelper` to create the corresponding event.

Let's start with the `schedule_tournament` function. This should take a tournament as an input, then _extract_ information from all its games, passing them to the `create_event` method of `EventBriteAPIHelper`.

The big question is: When should we call the `save_game` method? That is up to you to figre out. At the end of this step though you should be able to create a brand new tournament, then go to your [eventbrite page](https://www.eventbrite.com/organizations/home) and see all the corresponding events!

## Part 2: Updating events
Next step is to finish the `update_game_event` method.... but wait

wait wait wait

We may have a problem here. In order to update a game, we need to know it's event id! we could get the ids from the calls to `create_event`, but we don't right now! We have some work to do:
...
- Make sure that as you call `schedule_tournament`, each game is updated with its event id.
